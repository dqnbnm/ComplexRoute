...
if count_cities > 2: # если кол-во городов больше двух
    ...
    for i in range(count_cities - 2):
        if i == 0: # если первая итерация
            ...
            way_info1 = scheduling(api_key, cities[i], cities[i + 1], dates[i], city_code_dict, transfer_true, cord_stations)
            #CostProcessing.processing(way_info1)
        else:
            way_info1 = way_info
        way_info2 = scheduling(api_key, cities[i + 1], cities[i + 2], dates[i + 1], city_code_dict, transfer_true, cord_stations)
    ...
    way_info = scheduling(api_key, cities[0], cities[1], dates[0], city_code_dict, transfer_true, cord_stations) # считаем всевозможные маршруты
    if len(way_info) == 0:
        print("Маршруты не найдены")
